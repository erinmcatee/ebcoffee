...
@$link = mysql_connect( '192.168.8.225', 'fw0097247458', 'gIjmQzvLKbQg92drX9uXZsKcGJuRJd' );
if ( ! $link ) {
	die( sprintf( "Could not connect to the MySQL server: %s\n", mysql_error() ) );
}
echo "Connected successfully to the MySQL server\n";

@$db_selected = mysql_select_db( db1210563288_staging, $link );
if ( ! $db_selected ) {
	die ( sprintf( "Could not select the database '%s': %s\n", 'db1210563288_staging',  mysql_error() ) );
}
echo "Database selected successfully\n";

echo "\nChecking tables for errors:\n";
$tables = mysql_query( "SHOW TABLES LIKE 'wp_5br9ctys2q_%'" );
